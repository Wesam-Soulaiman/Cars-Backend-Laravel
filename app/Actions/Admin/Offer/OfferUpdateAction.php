...
class OfferUpdateAction
{
    public function __construct(protected OfferRepository $offerRepository) {}

    public function __invoke(Offer $offer, OfferRequest $request)
    {
        $store = Auth::guard('store')->user();

        if ($store instanceof Store && ! Product::where('store_id', $store->id)->where('id', $request->product_id)->where('id', $offer->product_id)->exists()) {
        ...
        return $this->offerRepository->updateOffer($offer, $request->validated());
    }
